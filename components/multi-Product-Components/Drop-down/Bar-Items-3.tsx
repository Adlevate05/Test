...
import type { MultiBar } from "../../../utils/types/app-context.types";

type PriceMode = "default" | "percentage" | "fixed";

export default function BarItems3() {
  const {
    selectedStyle,
    multiPackages,
    multiAddPackage,
  ...
  const handleUpdate = (id: string, patch: Partial<MultiBar>) => {
    if (patch.selectedByDefault) {
      multiPackages.forEach((p, i) => {
        if (p.selectedByDefault && p.id !== id) {
          multiUpdatePackage(i, { selectedByDefault: false });
        }
      });
    }

    const index = multiPackages.findIndex((p) => p.id === id);
    if (index >= 0) multiUpdatePackage(index, patch);
    if (patch.badgeStyle === "most-popular") multiUpdatePackage(index, { badgeText: "Most Popular" });
  };

  const handleAddBar = () => {
    const nextIndex = multiPackages.length + 1;
    multiAddPackage({
      title: `Pack`,
      quantity: nextIndex,
    });
  ...
  useEffect(() => {
    if (!openNewestNextRender || multiPackages.length === 0) return;
    const newest = multiPackages[multiPackages.length - 1];
    setOpenSections((prev) => ({ ...prev, [newest.id]: true }));
    setOpenNewestNextRender(false);
  }, [openNewestNextRender, multiPackages]);
              ...
              <Box padding="300">
                ...
                >
                  ...
                  <Text as="h3" variant="headingSm" fontWeight="bold">
                    Bar #{index + 1} - {bar.title} - {bar.quantity}
                  </Text>
                  ...
                  <div className="ml-auto">
                    <InlineStack gap="100" align="center">
                      <div className="flex gap-2">
                        ...
                        >
                          <button
                            onClick={() => multiMovePackageUp(index)}
                        ...
                        >
                          <button
                            onClick={() => multiMovePackageDown(index)}
                        ...
                        >
                          <button
                            onClick={() => multiRemovePackage(index)}
                            className="p-2 rounded-md flex items-center justify-center"
                          >
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              handleUpdate(bar.id, { quantity: value });
                            }}
                          />
                        </div>
                      </div>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              step={bar.priceMode === "percentage" ? 1 : 0}
