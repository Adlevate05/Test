...
import { MultiDealForm } from "app/components/multi-Product-Components/multi-deal-form";
import { parseMultiOptions, toAppContextInitial, toBlockStyle } from "app/utils/helpers";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const existing = await service.findById(numericId);
    ...
    const options = parseMultiOptions(existing?.configuration_block_options);
    ...
    const initial = {
      ...
      eligibilty: existing?.visibility_bundle === "bundle_specific" ? "bundle_specific" : "bundle_except",
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const dto = await parseDiscountDTO(request, "quantity-break-multi-product");

  const service = new DiscountService(admin, shopDomain, strategyMap);

  try {
    await service.update("quantity-break-multi-product", dto, numericId);
    return redirect("/app");
  } catch (e: any) {
    return json(
...
// ---- page wrapper ----
export default function MultiDealEdit() {
  ...
  return (
    <AppProvider initialState={initialState}>
      <MultiDealForm mode="edit" id={bundleId} />
    </AppProvider>
  );
}
